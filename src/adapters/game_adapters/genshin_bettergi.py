...
class GenshinBetterGIAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: 配置字典，包含genshin_path, bettergi_path等
        """
        super().__init__(config)
        self.genshin_path = Path(config.get('genshin_path', ''))
        self.bettergi_path = Path(config.get('bettergi_path', ''))
        ...
        self.image_templates = {
            # BetterGI初始状态
            'bettergi_initial_start_btn': config.get('templates_path', 'templates') + '/bettergi_initial_start_btn.png',
            'bettergi_dragon_btn_before': config.get('templates_path', 'templates') + '/bettergi_dragon_btn_before.png',
            # BetterGI点击一条龙后状态
            'bettergi_blue_play_btn': config.get('templates_path', 'templates') + '/bettergi_blue_play_btn.png',
            'bettergi_dragon_btn_after': config.get('templates_path', 'templates') + '/bettergi_dragon_btn_after.png',
            # 通用模板（备用）
            'bettergi_start_btn': config.get('templates_path', 'templates') + '/bettergi_start_btn.png',
            'bettergi_dragon_btn': config.get('templates_path', 'templates') + '/bettergi_dragon_btn.png',
            'bettergi_play_btn': config.get('templates_path', 'templates') + '/bettergi_play_btn.png',
            # 游戏相关
            'genshin_login_screen': config.get('templates_path', 'templates') + '/genshin_login_screen.png',
            'genshin_main_menu': config.get('templates_path', 'templates') + '/genshin_main_menu.png',
            'automation_complete': config.get('templates_path', 'templates') + '/automation_complete.png'
        }
    
    async def start(self):
        """启动适配器 - 启动游戏和脚本框架"""
        print("启动GenshinBetterGI适配器...")
        ...
        print("GenshinBetterGI适配器已启动")
        return True
    
    async def stop(self):
        """停止适配器 - 关闭游戏和脚本框架"""
        print("停止GenshinBetterGI适配器...")
        self.is_running = False
        
        await self.close_processes()
        print("GenshinBetterGI适配器已停止")
        return True
    ...
    async def execute(self, *args, **kwargs):
        ...
        result = await self.wait_for_completion(check_interval=check_interval, timeout=timeout)
